...
NvBool Synaptics_OneTouch_Open (NvOdmOneTouchDeviceHandle* hDevice, NvOdmOsSemaphoreHandle* hIntSema)
{
    ...
    if (!pConnectivity)
    {
        printk("[ONETOUCH] NvOdm Touch : pConnectivity is NULL Error \n");
        goto fail;
    }

    if (pConnectivity->Class != NvOdmPeripheralClass_HCI)
    {
        printk("[ONETOUCH] NvOdm Touch : didn't find any periperal in discovery query for touch device Error \n");
        goto fail;
    }
    ...
    if ((found & 3) != 3)
    {
        printk("[ONETOUCH] NvOdm Touch : peripheral connectivity problem \n");
        goto fail;
    }
...
// 20101120 hyeongwon.oh@lge.com power off when Onetouch close    
    ...
    {
        printk("[ONETOUCH] Synaptics Power fail \n");
        hTouch->VddId = 0xFF; 
    }
    ...
    if (!hTouch->hOdmI2c)
    {
        printk("[ONETOUCH] NvOdm Touch : NvOdmI2cOpen Error \n");
        goto fail;
    }
    ...
    if (!hTouch->hGpio)
    {        
        printk("[ONETOUCH] NvOdm Touch : NvOdmGpioOpen Error \n");
        goto fail;
    }
    ...
    if (!hTouch->hPin)
    {
        printk("[ONETOUCH] NvOdm Touch : Couldn't get GPIO pin \n");
        goto fail;
    }
